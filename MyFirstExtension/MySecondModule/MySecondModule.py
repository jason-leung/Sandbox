...
class MySecondModule(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.title = "Sphere ROI" # TODO make this more human readable by adding spaces
    self.parent.categories = ["Examples"]
    self.parent.dependencies = []
    self.parent.contributors = ["John Doe (AnyWare Corp.)"] # replace with "Secondname Lastname (Organization)"
...
class MySecondModuleWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    # parametersFormLayout.addRow("Output Model", self.outputLabel) 
...
class MySecondModuleLogic(ScriptedLoadableModuleLogic):
  ...
  def getSphereModel(self, markupsNode, output):
    import numpy as np
    if markupsNode.GetNumberOfMarkups() > 1:
      point1 = np.zeros(3)
      point2 = np.zeros(3)
      center = np.zeros(3)
      radius = 0

      markupsNode.GetNthFiducialPosition(markupsNode.GetNumberOfMarkups()-1,point1)
      markupsNode.GetNthFiducialPosition(markupsNode.GetNumberOfMarkups()-2,point2)
      ...
      output.SetAndObservePolyData(s.GetOutput())
  ...
  def run(self, inputMarkups, outputVolume, imageThreshold, enableScreenshots=0):
    ...
    """
    self.getSphereModel(inputMarkups, outputVolume)
    return True



class MySecondModuleTest(ScriptedLoadableModuleTest):
  """
  ...
  def runTest(self):
    ...
    """
    self.setUp()
    self.test_MySecondModule1()

  def test_MySecondModule1(self):
